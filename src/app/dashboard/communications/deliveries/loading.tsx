  ...
  return (
    <div className="space-y-6 animate-pulse" aria-busy="true">
      ...
      <div className="bg-white rounded-2xl border border-[rgba(7,26,40,0.08)] overflow-hidden">
        <div className="p-4 bg-[#f8f7f4] border-b border-[rgba(7,26,40,0.06)] grid grid-cols-8 gap-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-2 bg-slate-200 rounded-full" />
          ))}
        </div>
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="p-4 border-b border-[rgba(7,26,40,0.04)] grid grid-cols-8 gap-4">
