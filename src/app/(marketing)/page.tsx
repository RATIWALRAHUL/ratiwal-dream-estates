...
export default function HomePage() {
  ...
  return <>
    <section className="split-hero" aria-labelledby="home-hero-title">
      <Image src="/images/placeholders/property-placeholder.jpg" alt="Golden-hour aerial view of a planned township" fill priority sizes="100vw" className="hero-image object-cover"/>
      <div className="hero-panel">
        <div className="hero-copy"><p className="eyebrow">Trusted property consultancy</p><h1 id="home-hero-title">Find a place where your<br/><span>future begins.</span></h1><p>Verified land opportunities, clear guidance, and lifelong support for confident property decisions.</p><div className="hero-actions"><MagneticButton strength={8}><Link href="/properties" className="button-primary">Explore properties <ArrowRight size={17}/></Link></MagneticButton><MagneticButton strength={8}><Link href="/contact" className="button-ghost">Talk to an expert <ArrowRight size={17}/></Link></MagneticButton></div></div>
        <div className="hero-trust"><span><ShieldCheck size={16} strokeWidth={1.5}/>Verified land</span><span><FileCheck2 size={16} strokeWidth={1.5}/>Clear documentation</span><span><MapPin size={16} strokeWidth={1.5}/>Local expertise</span></div>
      </div>
      <div className="hero-proof"><ShieldCheck size={18} strokeWidth={1.5}/><span><strong><CountUp end={500} suffix="+"/></strong><small>verified opportunities</small></span></div>
    </section>
    ...
    <Reveal><section className="about-section" aria-labelledby="about-title"><div className="about-copy"><p className="eyebrow">About Ratiwal Dream Estates</p><h2 id="about-title">Property decisions,<br/>made with clarity.</h2><p>We help buyers and investors discover verified land opportunities across Rajasthan and Maharashtra. Every recommendation is guided by transparency, documentation, and a long-term view.</p><ul className="trust-list"><li><Check/>Clear, honest consultation</li><li><Check/>Carefully evaluated opportunities</li><li><Check/>Support from discovery to documentation</li><li><Check/>End-to-end assistance and legal support</li></ul><MagneticButton><Link href="/about" className="button-primary">Learn more about us <ArrowRight size={17}/></Link></MagneticButton></div><div className="about-visual"><div className="about-image"><Image src="/images/placeholders/insight-placeholder.jpg" alt="Architectural plans under professional review" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover"/></div><div className="floating-badge"><i><ShieldCheck size={18}/></i><span><strong>Verified &amp; Transparent</strong><small>Every opportunity reviewed</small></span></div><div className="metrics"><span><strong><CountUp end={10} suffix="+"/></strong><small>Years of<br/>Experience</small></span><span><strong><CountUp end={1} decimals={0} suffix="K+"/></strong><small>Happy<br/>Clients</small></span><span><strong><CountUp end={500} suffix="+"/></strong><small>Verified<br/>Projects</small></span><span><strong><CountUp end={100} suffix="%"/></strong><small>Transparency<br/>Always</small></span></div></div></section></Reveal>
    <PropertyAdvisorSection />
    <Reveal><section className="testimonials-section editorial-testimonials" aria-labelledby="testimonials-title"><div className="client-story-feature"><div className="client-heading"><p className="eyebrow">Client stories</p><h2 id="testimonials-title">Trusted through<br/>every step.</h2></div><article className="story-card story-featured"><Quote className="story-quote" aria-hidden="true"/><div className="stars" aria-label={`${testimonials[0].rating} out of 5 stars`}>{Array.from({length:testimonials[0].rating},(_,index)=><Star key={index} fill="currentColor"/>)}</div><p>“{testimonials[0].comment}”</p><div className="story-person"><span>{testimonials[0].name.charAt(0)}</span><div><strong>{testimonials[0].name}</strong><small>{testimonials[0].location}</small></div></div></article></div><div className="story-stack">{testimonials.slice(1).map((item)=><article className="story-card" key={item.id}><Quote className="story-quote" aria-hidden="true"/><div className="stars" aria-label={`${item.rating} out of 5 stars`}>{Array.from({length:item.rating},(_,index)=><Star key={index} fill="currentColor"/>)}</div><p>“{item.comment}”</p><div className="story-person"><span>{item.name.charAt(0)}</span><div><strong>{item.name}</strong><small>{item.location}</small></div></div></article>)}</div></section></Reveal>
