...
export function DirectCommunicationModel() {
  ...
  const renderNodeIcon = (idx: number) => {
    switch (idx) {
      case 0:
        return <Building className="w-6 h-6 text-[var(--ratiwal-blue)]" />;
      case 1:
        return <Layers className="w-6 h-6 text-[var(--cyan)]" />;
      case 2:
      default:
        return <UserCheck className="w-6 h-6 text-[var(--ratiwal-blue)]" />;
    }
  };
              ...
              <div className="relative p-7 sm:p-8 rounded-2xl sm:rounded-3xl bg-[var(--surface)] border border-[rgba(7,26,40,0.08)] h-full flex flex-col justify-between shadow-xs">
                <div>
                  <div className="flex items-center justify-between gap-3 mb-6">
                    <div className="w-12 h-12 rounded-xl bg-white text-[var(--ratiwal-blue)] flex items-center justify-center shadow-xs">
                      {renderNodeIcon(idx)}
                    </div>
                    <span className="px-3 py-1 rounded-full bg-white border border-[rgba(7,26,40,0.08)] text-[11px] font-bold uppercase tracking-wider text-[var(--ratiwal-blue)]">
                      {node.badge}
                    </span>
                  </div>
                ...
                {idx < 2 && (
                  <div className="hidden lg:flex absolute -right-4 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-white border border-[rgba(7,26,40,0.1)] items-center justify-center text-[var(--ratiwal-blue)] shadow-xs">
                    <ArrowRight size={14} />
                  </div>
                )}
              </div>
          ...
          <div className="p-6 sm:p-10 rounded-2xl sm:rounded-3xl bg-[var(--midnight)] text-white shadow-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {directCommunicationModel.diligencePoints.map((point, pIdx) => (
                ...
                >
                  <span className="w-5 h-5 rounded-full bg-[var(--ratiwal-blue)] text-white flex items-center justify-center flex-shrink-0 mt-0.5">
