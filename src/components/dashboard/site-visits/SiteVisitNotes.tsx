...
import { addSiteVisitNoteAction } from "@/lib/actions/site-visit.actions";
...
export function SiteVisitNotes({ visitId, notes, version }: SiteVisitNotesProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await addSiteVisitNoteAction(visitId, body, version);
      if (result.success) {
        setBody("");
      } else {
    ...
    <div className="bg-white rounded-2xl border border-[rgba(7,26,40,0.08)] shadow-xs p-5 space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        <textarea
          ...
          placeholder="Add an internal note regarding this visit… (never visible to visitor)"
          maxLength={5000}
          rows={3}
      ...
      {/* Notes list */}
      {sorted.length === 0 ? (
        <p className="text-xs text-[#647581] italic">No notes recorded yet.</p>
      ) : (
