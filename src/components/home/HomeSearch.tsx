...
function SearchDropdown({
  ...
  return (
    ...
    >
      ...
      {open && (
        <div className="dropdown-menu shadow-xl" role="listbox" aria-label={label}>
          {options.map((option) => (
            <button
              type="button"
              role="option"
            ...
            >
              <span className="font-bold text-[13.5px] sm:text-[14px]">{option.label}</span>
              {value === option.value && <Check size={16} strokeWidth={2.5} />}
            </button>
          ))}
        </div>
      )}
...
export function HomeSearch({ properties }: { properties: Property[] }) {
  ...
  const propertyTypes = useMemo<Option[]>(
    () => [
      ...
      ...Array.from(new Set(properties.map((p) => p.propertyType))).map((item) => ({
        label: item,
        value: item,
      })),
    ],
  ...
  const budgets = useMemo<Option[]>(
    () => [
      { label: "Any budget", value: "" },
      ...Array.from(new Set(properties.map((p) => p.priceLabel))).map((item) => ({
        label: item,
        value: item,
      })),
    ],
  ...
  const matches = useMemo(
    () =>
      properties.filter(
        (p) =>
          (!location || `${p.location} ${p.city} ${p.state}`.toLowerCase().includes(location.toLowerCase())) &&
          (!type || p.propertyType === type) &&
          (!budget || p.priceLabel === budget)
      ),
    [location, type, budget, properties]
  );
  ...
  return (
    <section className="search-region" aria-label="Property search">
