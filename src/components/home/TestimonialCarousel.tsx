...
export function TestimonialCarousel({ testimonials }: { testimonials: Testimonial[] }) {
  ...
  return <div className="carousel" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocus={() => setPaused(true)} onBlur={() => setPaused(false)} aria-roledescription="carousel" aria-label="Client testimonials">
    <div className="testimonial-track" style={{ "--active-slide": active } as React.CSSProperties}>{testimonials.map((item) => <article className="testimonial-card" aria-hidden={active !== testimonials.indexOf(item)} key={item.id}><div className="stars" aria-label={`${item.rating} out of 5 stars`}>{Array.from({ length: item.rating }, (_, i) => <Star key={i} size={15} fill="currentColor" />)}</div><Quote className="quote" aria-hidden="true"/><p>{item.comment}</p><div className="testimonial-person"><span aria-hidden="true">{item.name.replace(/\[.*?\]/g, "").trim().charAt(0) || "R"}</span><div><strong>{item.name}</strong><small>{item.role || item.location}</small></div></div></article>)}</div>
